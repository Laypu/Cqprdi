...
function myDataTableTr(row, columnidx, idx, filedname, tableid, type, fileditemclass, totalcnt) {
    ...
    if (type === 'delcheckbox') {
        ...
        if (row.VerifyStatus != undefined) {
            if (row.VerifyStatus == "已通過" || row.VerifyStatus == "未通過")
                ...
                return "<td class='text-center'><label class='mt-checkbox mt-checkbox-single mt-checkbox-outline'>" +
                    "<input type='checkbox' class='checkboxes chksel' disabled  value='" + row[tableid] + "'  index='" + idx + "'/><span></span></label></td>";

        } else if (row.VerifyStr != undefined) {

            if (row.VerifyStr == "已通過" || row.VerifyStr == "未通過" ) {
                return "<td class='text-center'><label class='mt-checkbox mt-checkbox-single mt-checkbox-outline'>" +
                    "<input type='checkbox' class='checkboxes chksel'  value='" + row[tableid] + "'  index='" + idx + "'/><span></span></label></td>";
            }
            else {
    ...
    } else if (type === 'linkMsg') {
        return "<td><a href='#' class='editMsg' value='" + row[tableid] + "'>" + row[filedname] + "</a></td>";
        ...
        return "<td class='text-center'>" + moment(row[filedname]).format('YYYY/MM/DD HH:mm')+"</td>";
    } else if (type === 'date') {
        return "<td class='text-center'>" + moment(row[filedname]).format('Y/MM/DD') + "</td>";
    } else if (type === 'textarea') {
        return "<td><textarea style='width:100%' rows='5' readonly>" + row[filedname] + "</textarea></td>";
    }
    else {
