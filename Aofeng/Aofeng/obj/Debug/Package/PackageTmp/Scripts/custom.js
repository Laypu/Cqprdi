function CreatePost(action, object) {
    var form = $("#CreatePost");
    var myform = null;
...
function RegisterClicklink(eventclick, eventitem, linkurl, obj, type) {
    ...
    $(eventclick).delegate((eventitem), "click", function () {
            ...
            if (type == "UpdateItem") {
              ...
              CreatePost(linkurl, obj);
            }
        });
}

function RegisterClicklinkMsg(eventclick, eventitem, linkurl, obj, type) {
    ...
    $(eventclick).delegate((eventitem), "click", function () {
        ...
        if (type == "UpdateItem") {
            ...
            var dialog = bootbox.confirm({
                title: '修改' + obj.item,
                message: "<p class='btn_03 form-control_1' style='margin-left:5px'>" + (obj.itemname === undefined ? obj.item : obj.itemname) + "</p>" +
                    "<textarea id='txt_name' class='form-control' type='text' style='width:400px;margin-left:5px;display:inline'>" + text +"</textarea>" +
                    "<span class='required' id='nameerror' style='color:red;display:none;margin-left:5px'>必須輸入！</span>",
                buttons: {
                    cancel: {
                        label: '<i class="fa fa-times"></i>取消'
...
function CallRemarkDialog(posturl, postobj) {
    var dialog = bootbox.confirm({
        title: (postobj.title === undefined ? '新增' + postobj.item : postobj.title),
        message: "<textarea stylw='width:100%' rows='5' id='txtRemark' class='form-control'></textarea>" +
            "<span class='required' id='nameerror' style='color:red;display:none;margin-left:5px'>必須輸入！</span>",
        buttons: {
            cancel: {
                label: '<i class="fa fa-times"></i>取消'
            ...
            if (result) {
                ...
                postobj.remark = _html.encode(CKEDITOR.instances.txtRemark.getData());
                debugger;
                $.ajax({
                    url: posturl,
                    data: postobj,
...
function RegisterSearchItem() {
    $(".search_icon").unbind('click').click(function () { var collapsed = $(this).find('i').hasClass('fa-search-minus'); $(".search_menu").slideToggle(); $('.search_icon').find('i').removeClass('fa-search'); $('.search_icon').find('i').addClass('fa-search-minus'); if (collapsed) $(this).find('i').toggleClass('fa-search-minus fa-2x fa-search fa-2x') });
    $("#btn_searchrefresh").unbind('click').click(function () { $("#searchForm :input").val(''); mytable.GetData(); });
}
